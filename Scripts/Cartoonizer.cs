...
namespace PhotoOrganizerScripts {
    class Cartoonizer : BaseScript {
        ...
        public Cartoonizer() {
            ...
            Description = "Transforms an image to a cartoon like version. The script is using the CodeProject.AI Cartoonizer sub module to process the images. Select images to process. If the output folder is specified, the resulting images are save to the specified folder. The five first images are automatically dispalyed.";
        }
        ...
        protected override bool ProcessImage(DB.Image image) {
            ...
            try {
                ...
                string resultingImage = Cartoonize(client, request).GetAwaiter().GetResult();
                ...
                LogView.AppendText($"Cartoonizer failed.\n{ex.Message}");
                ok = false;
            }
            return ok;
        }
